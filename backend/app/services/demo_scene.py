"""The demo scene: one mid-flight Chicago trip with a full plan layer.

Shared by scripts/seed.py (dev database) and POST /auth/demo (a fresh scene
per demo account). Dates are relative to today so the scene never goes stale.
...
async def build_demo_scene(session: AsyncSession, user: User) -> tuple[Trip, ...]:
    """Populate `user` (already flushed) with the demo trips; returns them."""
    today = date.today()
    ...
    for i, (starts, ends, title, location) in enumerate(cal_events):
        await session.execute(
            ...
            ),
            {
                "uid": user.user_id,
                "sid": source_id,
                # External ids are unique per source, and each demo account
                # gets its own sources, so a fixed prefix cannot collide.
                "tid": chicago.trip_id,
                "ext": f"seed_evt_{i}",
                "title": title,
                "loc": location,
